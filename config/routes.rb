...
Rails.application.routes.draw do
  ...
  authenticated :user do
    root to: redirect("/prompts"), as: :user_root
  end

  # Prompt projects
  resources :prompt_projects, only: %i[show index create update], path: "prompts" do
    resources :prompt_project_versions, path: "v" do
      get "playground" => "prompt_playground#show", as: :playground

      resources :ai_units, only: %i[create update]
      resources :executions, only: [:create]
      resources :variables_sets, only: [:new]
    end
  end

  resources :variables_sets, only: %i[index create edit update]

  # Public root page
  # This needs to go below user root route definition
