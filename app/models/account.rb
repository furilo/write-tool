...
class Account < ApplicationRecord
  ...
  validates :subdomain, exclusion: {in: RESERVED_SUBDOMAINS, message: :reserved},
    format: {with: /\A[a-zA-Z0-9]+[a-zA-Z0-9\-_]*[a-zA-Z0-9]+\Z/, message: :format, allow_blank: true}, uniqueness: {allow_blank: true}
  ...
  def transfer_ownership(user_id)
    ...
    Account::OwnershipNotification.with(account: self, previous_owner:).deliver_later(user)
  rescue
    false
  end
